...
function ExcavatorListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ExcavatorInterface[]>(
    ...
    () =>
      getExcavators({
        relations: ['user', 'invitation.count', 'project.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('excavator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/excavators/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('excavator', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            ...
            <NextLink href={`/excavators/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('excavator', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/excavators/edit/${record.id}`} passHref legacyBehavior>
                          <Button
