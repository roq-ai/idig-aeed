...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiMapPin, FiTruck } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['excavator', 'contributor', 'paid-subscriber'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ExcavatorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/excavators` : null),
    () => getExcavators({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Excavator',
      path: isTenantUser ? `/excavators/view/${data?.[0]?.id}` : '/excavators',
      entity: 'excavator',
    ...
    { name: 'Project', path: '/projects', entity: 'project', service: AccessServiceEnum.PROJECT, icon: FiMapPin },
